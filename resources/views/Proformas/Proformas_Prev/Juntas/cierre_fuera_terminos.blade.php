    ...
    <div id="footer">        
        <?php if($footer == null): ?>
            <div style="text-align:center;">
                <span style="color: #3C3C3C; margin-top:2px;">{{$nombre_afiliado}} - {{$tipo_documentos_benefi}} {{$nro_identificacion_benefi}} - SINIESTRO: {{$N_siniestro}} </span>
            </div>        
        <?php else: ?>
            <?php 
            ...
            <div class="footer_content" style="text-align:center;">
                <span style="position: absolute; width: 100%; text-align:center; top: 10px; left:0px; color:#4D4D4D; font-weight:bold; font-size: 11px;">
                    {{$nombre_afiliado}} - {{$tipo_documentos_benefi}} {{$nro_identificacion_benefi}} - SINIESTRO: {{$N_siniestro}}                     
                </span>
    ...
    <div class="container">
        ...
        <div style="float: right;text-align: right; width:97%; margin-top:30px; margin-bottom:30px;">
            <div class="col-6">
                ...
                @if ($tipo_afiliado == 27)
                    <br>
                    <span class="fuente_todo_texto">Beneficiario(a) <span class="negrita"><?php echo $nombre_afiliado.' '.$tipo_documentos_benefi.' '.$nro_identificacion_benefi;?></span></span>
                @endif              
            </div>
        </div>        
        <section class="fuente_todo_texto" style="margin-bottom: 2em; text-align: justify;">
            <?php 
                ...
                if (preg_match($patron1,$cuerpo) && preg_match($patron2,$cuerpo) && preg_match($patron3,$cuerpo)
                    ...
                    && preg_match($patron7,$cuerpo)
                ) {
                    $texto_modificado = str_replace('{{$nombre_afiliado}}', $nombre_afiliado, $cuerpo);
                    $texto_modificado = str_replace('{{$fecha_notificacion_afiliado}}', "<b>".$fecha_notificacion_afiliado."</b>", $texto_modificado);
                    $texto_modificado = str_replace('{{$fecha_radicacion_controversia_primera_calificacion}}', "<b>".$fecha_radicacion_controversia_primera_calificacion."</b>", $texto_modificado);
                    $texto_modificado = str_replace('{{$porcentajepcl}}', $porcentajepcl.' %', $texto_modificado);                    
