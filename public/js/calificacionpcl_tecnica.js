$(document).ready(function(){
    ...
    $("#decreto_califi").change(function(){
        alertas_informativas(
            'Verificar decreto de calificación',
            `Acaba de seleccionar el decreto <b>${$("#decreto_califi option:selected").text()}</b>, ¿está seguro de continuar la calificación con este decreto?`,
            '', 
            false,
            0,
            true,
    ...
    $(document).on('click', "#CorrespondenciaNotificacion", async function() {
        ...
        if (tipo_correspondencia.startsWith('afp_conocimiento')) {
            var datos_lista_tipos_documentos = guiasEntidadConocimiento (tipo_entidad_conocimiento, $("#Id_Evento_decreto").val(), 6, token, 'submodulo');
