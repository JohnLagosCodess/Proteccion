...
class CalificacionPCLController extends Controller
{
    ...
    public function guardarCalificacionPCL(Request $request){
        ...
        if($Accion_realizar == 6 || $Accion_realizar == 7 || $Accion_realizar == 8  || $Accion_realizar == 9 
        || $Accion_realizar == 10 || $Accion_realizar == 11 || $Accion_realizar == 12 || $Accion_realizar == 13
        || $Accion_realizar == 14 || $Accion_realizar == 17 || $Accion_realizar == 21 || $Accion_realizar == 23
        || $Accion_realizar == 25 || $Accion_realizar == 27 || $Accion_realizar == 30 || $Accion_realizar == 52 
        || $Accion_realizar == 53 || $Accion_realizar == 92){
    ...
    public function generarPdf(Request $request){
        ...
        elseif($request->tipo_documento_descarga_califi_editar == "Documento_PCL"){
            ...
            if($listado_documentos_solicitados){
                ...
                for ($i=0; $i < count($array_listado_documentos_solicitados); $i++) { 
                    $string_documentos_solicitados .= "<li><b>".$array_listado_documentos_solicitados[$i]["Descripcion"]."</b></li>";
                }
                $string_documentos_solicitados .= "</ul>";
            }else{
        ...
        elseif($request->tipo_documento_descarga_califi_editar == "No_Localizacion"){
            $dato_fecha_evento = sigmel_informacion_eventos::on('sigmel_gestiones')
            ->select('F_evento')
            ->where([['ID_evento', $ID_evento]])
            ->get();
            ...
            // Creación y guardado del pdf
            $pdf = app('dompdf.wrapper');
            $pdf->loadView('/Proformas/Proformas_Prev/PCL/no_localizacion', $data);

            $nombre_pdf = "No_Local_{$Id_comunicado}_{$Id_Asignacion}_{$N_identificacion}_{$indicativo}.pdf";
